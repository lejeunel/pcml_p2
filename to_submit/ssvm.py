...
class MySSVM(BaseEstimator):
    ...
    def fit(self, X, Y=None):
        ...
        self.prob_estimator.fit(X_feats_lin, Y_lin)

        #Reshape features and ground truths for ssvm
        X_crf = [(self.prob_estimator.predict_proba(X[i][0]), np.asarray(X[i][1]), np.asarray(X[i][2]).reshape(-1,1)) for i in range(len(X))]
